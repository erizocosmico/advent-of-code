...
let count_garbage_chars content = 
  let rec merge_garbage acc = function
    | Garbage chars -> List.append acc chars
    | Group inner -> inner |> List.fold_left ~init:acc ~f:merge_garbage
  in
  merge_garbage [] content |> List.length

let () =
  let stream = In_channel.read_all "09/input.txt" in
  stream |> parse_stream |> count_garbage_chars |> Stdio.printf "%d\n"
